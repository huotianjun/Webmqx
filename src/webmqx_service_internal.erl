...
service_test(_Body) -> 
    response_to_json([{<<"content-type">>, <<"text/html">>}], <<"Hello World">>).
...
internal_service2(#{<<"method">> := <<"list">>, <<"content">> := Path}) when is_binary(Path)->
    ...
    response_to_json([{<<"content-type">>, <<"text/html">>}], list_to_binary(ResultString));
internal_service2(#{<<"method">> := Method, <<"content">> := _}) ->
    response_to_json([{<<"content-type">>, <<"text/html">>}], <<"Unknown command : ",  Method/binary>>);

internal_service2(_) ->
    response_to_json([{<<"content-type">>, <<"text/html">>}], <<"Bad command format">>).
...
tsung_report2(#{<<"method">> := <<"tsung">>, <<"content">> := ReportFile}) when is_binary(ReportFile)->
    response_to_json([{<<"content-type">>, <<"text/html">>}], read_file(ReportFile)).
